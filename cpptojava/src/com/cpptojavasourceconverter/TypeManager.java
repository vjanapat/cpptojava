...
class TypeManager 
{
	...
	{
		StringTokenizer st = new StringTokenizer(filename,".cpp");
		
		String tempfilename = st.nextToken().toString();
		
		st = new StringTokenizer(tempfilename,"\\");
		
		while( st.hasMoreTokens())
			tempfilename = st.nextToken().toString();
	...
	private CppClass getClassToHouseGlobalDecls(String filename)
	{
		...
		if (cls == null)
		{
			...
			cls.name = getclassname(filename);
	...
	String cppToJavaType(IType type, TypeType tp) throws DOMException
	{
		if (type instanceof IBasicType)
		{
			...
			if (tp == TypeType.RAW)
				...
				return evaluateSimpleTypeBoxed(basic.getKind(), basic.isShort(), basic.isLongLong(), basic.isUnsigned());
			else
				return evaluateSimpleTypeBoxed(basic.getKind(), basic.isShort(), basic.isLongLong(), basic.isUnsigned());
		}
